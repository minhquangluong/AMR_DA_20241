...
String binaryArray[6] = {"101", "100", "110", "010", "011", "001"};
...
void setup() {
  ...
  digitalWrite(zfPinLeft, LOW);               // Thiết lập chiều quay ban đầu
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (input.length() > 0 && isDigit(input[0]) || input[0] == '-') {
      targetAngularVelocity = input.toFloat();
    }

    if (targetAngularVelocity == 0) {
      while (pwmValue > MIN_PWM) {
      pwmValue -= 2;
      analogWrite(vrPinLeft, pwmValue);
    }
    digitalWrite(elPinLeft, LOW);
    integral = 0;
    previousError = 0;
    initialSet = false;
    } else {
      ...
      if (targetAngularVelocity < 0) {
        ...
        targetAngularVelocity=abs(targetAngularVelocity);
      } else {
  ...
  if ((unsigned long)(currentTime - previousTime) >= interval) {
    int deltaCount = abs(count_l - previousCount_l);
